...
## 概要
テスト手順自動生成システムのベースとなるFastAPIアプリケーションを構築する。**MVP段階（社内限定運用）**に必要な最小限の機能から始め、将来的な外部公開に備えた拡張可能な設計とする。

**実装フェーズ**：
- **Phase 1 (MVP)**: 社内ネットワーク限定、HTTP通信、基本機能
- **Phase 2 (外部公開)**: HTTPS、セキュリティ強化、詳細監視

## 詳細
- FastAPIプロジェクトの初期セットアップ
- ディレクトリ構造の設計（app/, tests/, config/ など）
- 基本的な設定管理（環境変数、ログ設定、プロンプトバージョン管理）
- ヘルスチェックエンドポイント（/healthz、/readyz）の実装
- CORS設定、セキュリティヘッダーの設定
- OpenAPI（Swagger）ドキュメント設定
- trace_id生成機能の実装（全リクエスト追跡用）
- JSON Schema検証基盤の実装（テスト手順生成API入出力スキーマ対応）
- レイテンシ計測機能の実装（応答時間測定、メタデータ出力対応）
- TLS/HTTPS設定（セキュリティ要件）
- エラーハンドリング機能の実装（統一フォーマット、HTTPステータス標準化）
- リクエスト制限機能の実装（手順辞書データ対応、レート制限、サイズ制限、タイムアウト）
...
- 環境変数設定ファイル（.env）の作成
  ...
  - PROMPT_TEMPLATE_PATH（プロンプトテンプレートファイルパス、デフォルト: templates/）
...
## Task 4: セキュリティ設定（MVP段階：基本設定のみ）

### 実装ステップ（MVP段階）
- **基本セキュリティミドルウェアの実装**
  - 最小限のセキュリティチェック機能により、社内ネットワーク環境での基本的な安全性を確保する
- **Swagger UI用最小限CORS設定（開発時のみ）**
  - 開発効率とセキュリティのバランスを取り、開発時のAPI文書化アクセスを安全に提供する
- **環境変数による段階的機能制御**
  - SECURITY_HEADERS_ENABLED（デフォルト: false）による外部公開時の機能切り替え準備

### 外部公開時の追加実装（Phase 2）
- セキュリティヘッダー設定（HSTS、CSP等）: ブラウザレベルでのセキュリティ強化
- 高度なリクエストヘッダー検証: 不正なヘッダーの早期検出
...
- テスト手順生成APIリクエストボディの検証機能
  - `test_case_text`（自然言語テストケース）の形式検証
  - `default_actions`（標準手順辞書）のスキーマ検証
  - `user_defined_actions`（ユーザー定義手順）のスキーマ検証
  - `options`（モデル、ロケール、最大ステップ数）の妥当性確認
- レスポンスボディの検証機能
  - `trace_id`, `summary`, `steps`の必須フィールド検証
  - `meta`フィールド（model, prompt_version, latency_ms）の構造検証
  - `nearest_candidates`の近似候補データ構造検証
- エラーメッセージの標準化
...
- メタデータ出力機能の実装
  - `latency_ms`フィールドでのレスポンス時間出力
  - 使用モデル、プロンプトバージョンと組み合わせた統合メタデータ提供
- パフォーマンス統計の収集機能
...
## Task 9: TLS/HTTPS設定（外部公開時の実装：Phase 2で対応）

### MVP段階での対応
- **HTTP通信での運用**: 社内ネットワーク限定のため、HTTP通信で十分な安全性を確保
- **段階的実装の準備**: 環境変数TLS_ENABLED（デフォルト: false）による将来の切り替え準備
- **設定インターフェースの準備**: 外部公開時のTLS設定を容易にするための基盤実装

### 外部公開時の実装（Phase 2）
- **SSL証明書の設定機能**: 通信の暗号化によるデータ送受信時の盗聴防止と通信相手の認証
- **HTTPS強制リダイレクト機能**: HTTP通信の自動HTTPS転送によるセキュリティポリシーの強制
- **TLS設定の最適化**: 最新の暗号化プロトコル使用によるセキュリティレベルの向上
- **セキュリティプロトコルの設定**: 脆弱なプロトコルの無効化による既知の脆弱性対策
...
- リクエストサイズ制限機能
  - 手順辞書データ（約80件の標準手順＋ユーザー定義手順）を含む大容量リクエストに対応したサイズ制限
  - JSON構造のネストデータに対する適切なサイズ算出とメモリ枯渇防止
- タイムアウト設定機能
